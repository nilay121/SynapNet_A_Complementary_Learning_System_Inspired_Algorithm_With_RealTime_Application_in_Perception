...
class utility_funcs:
    def buffer_dataGeneration(digit, experience,device, batch_size,model,num_examples,numbOf_orgExamples):
    ...
    def get_dataBuffer(buffer_data, buffer_labels, size,device,transform= None):
        '''
    ...
    def benchmarkDataPrep(experience, device, buffer_data=[],buffer_label=[]):
        seq_len = 1
        if len(buffer_data) and len(buffer_label) > 0:
            ...
            size=len(total_bufferLength),device=device,)

            buffer_inputs = buffer_inputs.cpu().detach().numpy()
            buffer_inputs = buffer_inputs.reshape(-1,buffer_inputs.shape[1], seq_len)
            buffer_labels = buffer_labels.cpu().detach().numpy()

            train_data_loader = DataLoader(train_dataset, num_workers=4, batch_size=total_dataLength,shuffle=False) 
            for data in  train_data_loader:
                input_dataBT = data[0]
                input_dataBT = input_dataBT.reshape(-1, input_dataBT.shape[1], seq_len).to(device)
    ...
    def barPlotMeanPred(meanBenchEWC,meanBenchLWF,meanBenchSI,meanBenchNaive,meanBenchJoint,stdBenchEWC,stdBenchLWF,stdBenchSI,stdBenchNaive,stdBenchJoint,n_experinces):
        ...
        ax.set_title("Object Classification Benchmark")
        ...
        fig.tight_layout()
        plt.show()
        plt.savefig("tactile/tactileSensor_after3runsWith80.png")
    ...
    def train(self,train_data):
        ...
        for epochs in range(self.epochs):
            ...
            for data, input_labels in loader_loop:
                ...
                data = data.reshape(-1, data.shape[1], self.seq_len)
        ...
        with torch.no_grad():
            for experiences in test_stream:
                ...
                for data in tqdm(eval_data_loader):
                    ...
                    input_dataBT = input_dataBT.reshape(-1, input_dataBT.shape[1], self.seq_len)   
